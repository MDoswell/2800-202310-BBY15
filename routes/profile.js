...
router.get('/profile', sessionValidation, async (req, res) => {
  ...
  const userInfo = await userCollection
    .findOne({ name: loggedInUser }, { projection: { name: 1, user_type: 1, email: 1, experience: 1, weight: 1 } });
  ...
  if (userInfo) {
    ...
    const errorMessage = '';
    res.render("profile", { name, email, user_type, experience, weight, errorMessage });
  } else {
